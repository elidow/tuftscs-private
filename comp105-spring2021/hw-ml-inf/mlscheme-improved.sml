...
(*****************************************************************)
(*                                                               *)
(*   \FOOTNOTESIZE SHARED: NAMES, ENVIRONMENTS, STRINGS, ERRORS, *)
(*   PRINTING, INTERACTION, STREAMS, \&\ INITIALIZATION          *)
(*                                                               *)
(*****************************************************************)
...
(*****************************************************************)
(*                                                               *)
(*   LEXICAL ANALYSIS AND PARSING FOR \USCHEME, PROVIDING        *)
(*   [[FILEXDEFS]] AND [[STRINGSXDEFS]]                          *)
(*                                                               *)
(*****************************************************************)
 ...
 *           Given an expression e and a name of the variable name, freeIn 
 *           returns whether its free  *)

fun freeIn (LITERAL v) name               = false
  | freeIn (VAR n) name                   = n = name
  | freeIn (SET (n, exp1)) name           = n = name orelse freeIn exp1 name
  | freeIn (IFX (exp1, exp2, exp3)) name  = 
           if freeIn exp1 name then freeIn exp2 name else freeIn exp3 name
  | freeIn (WHILEX (exp1, exp2)) name     = 
           freeIn exp1 name orelse freeIn exp2 name
  | freeIn (BEGIN xs) name                = 
           foldl (fn (exp, acc) => acc orelse freeIn exp name) false xs
  | freeIn (APPLY (exp1, xs)) name        = 
           (freeIn exp1 name) orelse 
               (foldl (fn (exp, acc) => acc orelse freeIn exp name) false xs)
  | freeIn (LETX (LET, xs, exp1)) name    = 
            ((freeIn exp1 name) andalso 
                (not (member name (fst (ListPair.unzip xs))))) 
                orelse (foldl (fn (exp, acc) => acc 
                orelse freeIn exp name) false (snd (ListPair.unzip xs)))
  | freeIn (LETX (LETREC, xs, exp1)) name = 
             ((freeIn exp1 name) 
             orelse (foldl (fn (exp, acc) => acc orelse freeIn exp name) false 
             (snd (ListPair.unzip xs)))) andalso 
             (not (member name (fst (ListPair.unzip xs))))
  | freeIn (LETX (LETSTAR, [], e)) y      = freeIn e y
  | freeIn (LETX (LETSTAR, b::bs, e)) y   = 
             freeIn (LETX (LET, [b], LETX (LETSTAR, bs, e))) y
  | freeIn (LAMBDA (xs, exp1)) name       = 
             (freeIn exp1 name) andalso (not (member name xs))

        (* unit tests *)
        val y = "COMP"
        val x = "105"
        val () =
            Unit.checkExpectWith Bool.toString "freeIn: free"
            (fn () => freeIn (LAMBDA ([y], VAR x)) x)
            true
        val () =
            Unit.checkExpectWith Bool.toString 
            "freeIn: appears in the expression, but is not"
            (fn () => freeIn (LAMBDA ([y], VAR x)) y)
            false

(* part b *)

(* (improve : (name list * exp) * 'a env -> (name list * exp) * 'a env)
 *             Given a pair consisting of a LAMBDA body lam and an environment 
 *             rho, improve returns a pair of lam and an evironment of all 
 *             the free variables of lambda  *)

fun improve ((xs, exp), rho) = 
    ((xs, exp), List.filter (fn pair => freeIn (LAMBDA(xs, exp)) 
         (fst pair)) rho)
...
(* if any expression in [[values]] is not a [[lambda]], reject the [[letrec]] 
 * S367a *)
...
(*****************************************************************)
(*                                                               *)
(*   IMPLEMENTATIONS OF \USCHEME\ PRIMITIVES AND DEFINITION OF   *)
(*   [[INITIALBASIS]] *)
(*                                                               *)
(*****************************************************************)
...
(*****************************************************************)
(*                                                               *)
(*   FUNCTION [[RUNAS]], WHICH EVALUATES STANDARD INPUT GIVEN    *)
(*   [[INITIALBASIS]]                                            *)
(*                                                               *)
(*****************************************************************)
...
(*****************************************************************)
(*                                                               *)
(*   CODE THAT LOOKS AT COMMAND-LINE ARGUMENTS AND CALLS         *)
(*   [[RUNAS]] TO RUN THE INTERPRETER                            *)
(*                                                               *)
(*****************************************************************)
