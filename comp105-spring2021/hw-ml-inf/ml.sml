...
fun asFuntype (CONAPP (TYCON "function",
      ...
      SOME (args, result)
  | asFuntype _ = NONE
fun reftype ty = CONAPP (TYCON "ref", [ty])
...
fun solve TRIVIAL     = idsubst
  | solve (c1 /\ c2)  = 
      let val first   = solve c1
          val second  = solve (consubst first c2)
      in  compose (second, first)
      end
  (* TYVAR/any *)
  | solve (TYVAR a ~ any) =
      (case any
       of TYVAR a1 => a |--> TYVAR a1
        | _        => 
          let val free = freetyvars any                                       
          in if member a free then
               unsatisfiableEquality (TYVAR a, any)                
             else a |--> any
          end)
  | solve (any ~ TYVAR a) = solve (TYVAR a ~ any)
  (* TYCON/TYCON *)
  | solve (TYCON t1 ~ TYCON t2) = 
      if eqType (TYCON t1, TYCON t2) then
        idsubst
      else
        unsatisfiableEquality (TYCON t1, TYCON t2)
  (* CONAPP/CONAPP *)
  | solve (CONAPP (t1, []) ~ CONAPP (t2, []))         = solve (t1 ~ t2)
  | solve (CONAPP (t1, t::ts) ~ CONAPP (t2, t'::ts')) =
      solve (t1 ~ t2 /\ (CONAPP (t, ts) ~ CONAPP (t',ts'))) 
  (* TYCON/CONAPP / others *)
...
fun solutionEquivalentTo (c, theta) = eqsubst (solve c, theta)
        ...
        val () = Unit.checkAssert 
                    "bool ~ bool is solved by the identity substitution"
                    (fn () => solutionEquivalentTo 
                      (booltype ~ booltype, idsubst))
        ...
        (* additional tests *)
        val () = Unit.checkAssert 
                    "'a ~ 'b and 'b ~ bool is solved by 'b |--> bool"
                    (fn () => hasSolution 
        ...
        (* /\ and TRIVIAL *)
        (* 'a ~ 'b /\ 'b ~ bool *)
        val () = Unit.checkAssert "'CONJUNCTION 1 is solved"
                    (fn () => solutionEquivalentTo 
                        (TYVAR "'a" ~ TYVAR "'b" /\ TYVAR "'b" ~ booltype,
                        compose("'b" |--> booltype, "'a" |--> TYVAR "'b")))
        (* 'b ~ bool /\ 'b ~ 'a *)
        val () = Unit.checkAssert "'CONJUNCTION 2 is solved"
                    (fn () => solutionEquivalentTo 
                        (TYVAR "'b" ~ booltype /\ TYVAR "'b" ~ TYVAR "'a",
                        compose("'b" |--> booltype, "'a" |--> booltype)))
        (* 'b ~ bool /\ 'b ~ 'a /\ 'a ~ bool *)
        val () = Unit.checkAssert "'CONJUNCTION 3 is solved"
                    (fn () => solutionEquivalentTo 
                        (TYVAR "'b" ~ booltype /\ TYVAR "'b" ~ TYVAR "'a" 
                            /\ TYVAR "'a" ~ booltype,
                        compose("'b" |--> booltype, "'a" |--> booltype)))
        (* TRIVIAL /\ 'a ~ bool *)
        val () = Unit.checkAssert "TRIVIAL conjuction is solved by 'a |--> 'b"
                    (fn () => solutionEquivalentTo 
                        (TRIVIAL /\ TYVAR "'a" ~ booltype,
                        "'a" |--> booltype))
        (* TRIVIAL *)
        val () = Unit.checkAssert "TRIVIAL is solved by idsubst"
                    (fn () => solutionEquivalentTo (TRIVIAL, idsubst))

        (* TYVAR/TYVAR  *)
        (* 'a ~ 'a *)
        val () = Unit.checkAssert "'a ~ 'a is solved by 'a |--> 'a"
        ...
        (* 'a ~ 'a *)
        val () = Unit.checkAssert "'a ~ 'a is solved by 'a |--> 'a"
                    (fn () => solutionEquivalentTo (TYCON "'a" ~ TYVAR "'a",
                        "'a" |--> TYCON "'a"))

        (* TYVAR/CONAPP *)
        (* 'a ~ list int *)
        val () = Unit.checkAssert "'a ~ list int is solved by 'a |--> list int"
                    (fn () => solutionEquivalentTo (TYVAR "'a" ~  
                        listtype inttype, "'a" |--> listtype inttype))
        (* list bool ~ 'a *)
        val () = Unit.checkAssert 
                   "list bool ~ 'a is solved by 'a |--> list bool"
                    (fn () => solutionEquivalentTo 
                        (listtype booltype ~ TYVAR "'a", 
                        "'a" |--> listtype booltype))
        (* 'a ~ list list int  *)
        val () = Unit.checkAssert 
                   "'a ~ list list int is solved by 'a |--> list list int"
                    (fn () => solutionEquivalentTo
                        (listtype (listtype inttype) ~ TYVAR "'a", 
                        "'a" |--> listtype (listtype inttype)))
        (* 'a ~ list 'a *)
        val () = Unit.checkAssert "'a ~ list 'a cannot be solved"
                    (fn () => hasNoSolution (TYVAR "'a" ~ 
                        listtype (TYVAR "'a")))
        (* 'a ~ list list 'a *)
        val () = Unit.checkAssert "'a ~ list list 'a cannot be solved"
                    (fn () => hasNoSolution
                        (TYVAR "'a" ~ listtype (listtype (TYVAR "'a"))))

        (* TYCON/CONAPP *)
        (* list bool ~ bool *)
        val () = Unit.checkAssert "list bool ~ bool cannot be solved"
                    (fn () => hasNoSolution (listtype booltype ~ booltype))
        (* bool ~ list int *)
        val () = Unit.checkAssert "bool ~ list int cannot be solved"
                    (fn () => hasNoSolution (booltype ~ listtype inttype))
        ...
        (* list bool ~ list bool *)
        val () = Unit.checkAssert 
                    "list bool ~ list bool is solved by solved idsubst"
                    (fn () => solutionEquivalentTo 
                      (listtype booltype ~ listtype booltype, idsubst))
        (* list bool ~ list 'a *)
        val () = Unit.checkAssert 
                    "list bool ~ list 'a is solved by 'a |--> bool"
                    (fn () => solutionEquivalentTo 
                        (listtype booltype ~ listtype (TYVAR "'a"),
                        "'a" |--> booltype))
        (* list 'a ~ list int *)
        val () = Unit.checkAssert "list 'a ~ list int is solved by 'a |--> int"
                    (fn () => solutionEquivalentTo 
                        (listtype (TYVAR "'a") ~ listtype inttype,
                        "'a" |--> inttype)) 
        (* list int ~ list bool *)         
        val () = Unit.checkAssert "list int ~ list bool cannot be solved"
                    (fn () => hasNoSolution 
                        (listtype inttype ~ listtype booltype))
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 448e *)
      fun literal (SYM s)         = (symtype       , TRIVIAL)
        | literal (NUM n)         = (inttype       , TRIVIAL)
        | literal (BOOLV b)       = (booltype      , TRIVIAL)
        | literal NIL             = (listtype alpha, TRIVIAL)
        | literal (PAIR (v, NIL)) =
            let val (vtype, c)    = literal v
            in  (listtype vtype, c)
            end
        | literal (PAIR (v, v'))  =
            let val (vtype, c)    = literal v
                val (vtypes, c')  = literal v'
            in  if eqType (vtypes, listtype vtype) then
                  (vtypes, c /\ c')
                else
                  raise TypeError "PAIR: v and v' are not same type"
            end
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        = 
            let val (tau1, c1)  = ty e1
                ...
                val (tau3, c3)  = ty e3
                val c           = 
                  c1 /\ c2 /\ c3 /\ tau1 ~ booltype /\ tau2 ~ tau3
            in  if hasSolution c then
                  (tau2, c)
                else raise TypeError "IFX: No Solution"
            end
        | ty (BEGIN es)                = 
            (case map ty es
            of [] => (unittype, TRIVIAL)
             | types => 
                 let val (taus, cs)  = ListPair.unzip types
                     val constraints = conjoinConstraints cs
                     val (t, c)      = List.last types
                 in  if hasSolution constraints then
                       (t, constraints)
                     else raise TypeError "BEGIN: No Solution"
                end)
        | ty (LAMBDA (formals, body))  =
            let val ext_gamma = foldl (fn (x, g) => bindtyscheme 
                                        (x, FORALL([x],freshtyvar ()), g)) 
                                        Gamma formals
                val (taubody, c)  = typeof(body, ext_gamma)
            in (funtype (map (fn (y) => 
                          freshInstance(findtyscheme (y, ext_gamma))) formals,
                          taubody), c)
            end
        | ty (LETX (LET, bs, body))    = 
                (* C *)
           let  val (names, es)  = ListPair.unzip bs
                val estypes      = map ty es 
                val (taus, cs)   = ListPair.unzip estypes
                val c            = conjoinConstraints cs
                (* C' *)
                val theta        = solve c
                val alphadomain  = inter (dom theta, freetyvarsGamma Gamma)
                val cs'          = map (fn x => 
                                          (TYVAR x) ~ 
                                            (tysubst theta (TYVAR x)))
                                          alphadomain
                val c'          = conjoinConstraints cs'
                (* generalize *)
                val sigmas      = map (fn tau => generalize (tysubst theta tau,
                                        union(freetyvarsGamma Gamma, 
                                        freetyvarsConstraint c'))) taus
                (* extended gamma *)
                val ext_gamma   = ListPair.foldlEq
                                    (fn (n, s, g) => bindtyscheme (n, s, g))
                                    Gamma (names, sigmas)
                val (t, cb)     = typeof(body, ext_gamma)
            in  (t, cb /\ c')
            end
        | ty (LETX (LETREC, bs, body)) =
                (* check lambda expressions *)
            let fun lambdaexp (LAMBDA _) = true
                  | lambdaexp _          = false
                (* distinct and fresh alphas *)
                val (names, es)  = ListPair.unzip bs
                val namealphas   = map (fn x => (x, freshtyvar ())) names
                val new_gamma    = foldl 
                                    (fn ((x, a), g) => bindtyscheme 
                                      (x, FORALL([x], a), g))
                                      Gamma namealphas
                (* Cr *)
                val estypes      = map (fn e => typeof (e, new_gamma)) es
                val (taus, cs)   = ListPair.unzip estypes
                val cr           = conjoinConstraints cs
                (* C *)
                val alphas       = map (fn (x, a) => a) namealphas
                val alphacs      = ListPair.foldlEq 
                                     (fn (t, a, emp) => (a ~ t) /\ emp)
                                     TRIVIAL (taus, alphas)
                val c            = cr /\ alphacs
                (* C' *)
                ...
                val ext_gamma    = ListPair.foldlEq
                                    ...
                                    Gamma (names, sigmas)
                val (t, cb)      = typeof(body, ext_gamma)
            in  if List.all (fn x => lambdaexp x) es then
                  (t, cb /\ c')
                else raise TypeError "LETREC: body needs to be lambda"
            end
(* type declarations for consistency checking *)
  ...
  let fun addPrim ((name, prim, tau), (Gamma, rho)) = 
      ...
      val primBasis = foldl addPrim (emptyTypeEnv, emptyEnv) 
                         ((* primitives for \nml\ and \uml\ [[::]] 451b *)
                          ...
                          ("fst",  unaryOp
                                     (fn (PAIR (a, _)) => a
                                       | NIL => raise RuntimeError "empty pair"
                                       | _   => raise BugInTypeInference
                                                     "not a pair"),
